...
class FlexAttnFunc(nn.Module):
    ...
    def init_mask(
        ...
        if mask_shape is not None:
            ...
            # mask tokens share the same seq_ids and frame_ids as video tokens.
            ...
            seq_ids = torch.cat([latent_seq_id] * 4 + [action_seq_id] * 2)
            vid_fid  = latent_frame_id // chunk_size * 2
            act_fid  = action_frame_id // chunk_size * 2 + 1
            frame_ids = torch.cat([vid_fid] * 4 + [act_fid] * 2)
            noise_ids = torch.cat([
                torch.zeros_like(latent_frame_id),   # vid_noisy   noise_id=0
                torch.ones_like(latent_frame_id),    # vid_cond    noise_id=1
                torch.zeros_like(latent_frame_id),   # mask_noisy  noise_id=0
                torch.ones_like(latent_frame_id),    # mask_cond   noise_id=1
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.mask_proj_out = nn.Linear(inner_dim,
                                       out_channels * math.prod(patch_size))
    ...
    def create_empty_cache(self, cache_name, attn_window,
                           ...
                           device, dtype, batch_size, has_mask=False):
    ...
    def forward_train(self, input_dict):
        # B2: backbone sees [vid_noisy | vid_cond | mask_noisy | mask_cond | act_noisy | act_cond].
        # mask tokens share the same spatial grid and timesteps as video tokens.
        # FlexAttn's _get_mask_mod is unchanged; init_mask is extended with mask_shape
        # to add mask entries to seq_ids/frame_ids/noise_ids automatically.
        for key in ('noisy_latents', 'latent'):
            input_dict['latent_dict'][key] = input_dict['latent_dict'][key].to(torch.bfloat16)
            input_dict['action_dict'][key] = input_dict['action_dict'][key].to(torch.bfloat16)
            input_dict['mask_dict'][key]   = input_dict['mask_dict'][key].to(torch.bfloat16)

        latent_dict = input_dict['latent_dict']
        mask_dict   = input_dict['mask_dict']
        action_dict = input_dict['action_dict']
        batch_size  = latent_dict['noisy_latents'].shape[0]

        # ── Embed all six streams ────────────────────────────────────────────
        vid_noisy_hs  = self._input_embed(latent_dict['noisy_latents'], 'latent').flatten(0, 1)[None]
        vid_cond_hs   = self._input_embed(latent_dict['latent'],        'latent').flatten(0, 1)[None]
        msk_noisy_hs  = self._input_embed(mask_dict['noisy_latents'],   'latent').flatten(0, 1)[None]
        msk_cond_hs   = self._input_embed(mask_dict['latent'],          'latent').flatten(0, 1)[None]
        act_noisy_hs  = self._input_embed(action_dict['noisy_latents'], 'action').flatten(0, 1)[None]
        act_cond_hs   = self._input_embed(action_dict['latent'],        'action').flatten(0, 1)[None]
        text_hs       = self._input_embed(latent_dict['text_emb'],      'text').flatten(0, 1)[None]

        # Joint sequence: [vid_noisy | vid_cond | mask_noisy | mask_cond | act_noisy | act_cond]
        hidden_states = torch.cat(
            [vid_noisy_hs, vid_cond_hs, msk_noisy_hs, msk_cond_hs, act_noisy_hs, act_cond_hs],
            dim=1,
        )

        # ── Positional embeddings ────────────────────────────────────────────
        # mask tokens use the same spatial grid as video tokens
        ...
        full_grid_id   = torch.cat([latent_grid_id] * 4 + [action_grid_id] * 2, dim=2)
        ...
        vid_ts  = torch.cat([latent_dict['timesteps'].flatten(0, 1),
                              latent_dict['cond_timesteps'].flatten(0, 1)])[None]
        msk_ts  = torch.cat([mask_dict['timesteps'].flatten(0, 1),
                              mask_dict['cond_timesteps'].flatten(0, 1)])[None]
        act_ts  = torch.cat([action_dict['timesteps'].flatten(0, 1),
                              action_dict['cond_timesteps'].flatten(0, 1)])[None]
        # combine: [vid_noisy|vid_cond|mask_noisy|mask_cond] → single _time_embed call
        latent_msk_ts = torch.cat([vid_ts, msk_ts], dim=1)

        latent_temb, latent_timestep_proj = self._time_embed(
            latent_msk_ts, L_H, L_W, dtype=hidden_states.dtype, action_mode=False)
        ...
        split_list = [
            vid_noisy_hs.shape[1],
            vid_cond_hs.shape[1],
            msk_noisy_hs.shape[1],
            msk_cond_hs.shape[1],
            act_noisy_hs.shape[1],
            act_cond_hs.shape[1],
            padded_length,
        ]

        # ── FlexAttn mask (extended to include mask token positions) ─────────
        FlexAttnFunc.init_mask(
            ...
            mask_shape=mask_dict['noisy_latents'].shape,  # B2 extension
        )

        # ── Backbone (frozen) ────────────────────────────────────────────────
        for block in self.blocks:
            hidden_states = block(hidden_states, text_hs, timestep_proj, rotary_emb,
                                  update_cache=False)
        ...
        # ── Split and project ────────────────────────────────────────────────
        vid_feat, _, msk_feat, _, act_feat, _, _ = torch.split(hidden_states, split_list, dim=1)
        ...
        mask_hidden_states = self.mask_proj_out(msk_feat)   # trainable, operates on mask-stream features
        mask_hidden_states = rearrange(mask_hidden_states,
                                       '1 (b l) (n c) -> b (l n) c',
                                       n=math.prod(self.patch_size), b=batch_size)
    ...
    def forward(
        ...
        if action_mode:  # action input emb
            ...
            for block in self.blocks:
                latent_hidden_states = block(latent_hidden_states, text_hidden_states,
        ...
        if noisy_mask is not None:
            msk_hs = self._input_embed(noisy_mask.to(vid_hs.dtype), 'latent')  # [B, L, D]
        ...
        ts_vid = torch.repeat_interleave(input_dict['timesteps'], spatial_per_frame, dim=1)
        if noisy_mask is not None:
        ...
        vid_out = rearrange(vid_out, 'b l (n c) -> b (l n) c', n=math.prod(self.patch_size))

        if noisy_mask is not None:
            msk_feat = latent_hidden_states[:, vid_L:]
            msk_out  = self.mask_proj_out(msk_feat)
            msk_out  = rearrange(msk_out, 'b l (n c) -> b (l n) c', n=math.prod(self.patch_size))
